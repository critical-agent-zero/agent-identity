...
5. Run the **deploy** workflow from the Actions tab. The job summary lists the remaining manual steps (DNS MX record, SES domain verification, fleet key); stack outputs are not published on this public repo — read them with `aws cloudformation describe-stacks --stack-name AgentIdentity --query 'Stacks[0].Outputs'`. The workflow activates the SES receipt rule set automatically.
